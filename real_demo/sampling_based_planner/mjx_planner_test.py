...
class cem_planner():
    ...
    def __init__(self, model=None, num_dof=None, num_batch=None, num_steps=None, timestep=None, maxiter_cem=None,
                 num_elite=None,
        ...
        self.num_total_constraints = (
                self.num_vel_constraints + self.num_acc_constraints + self.num_jerk_constraints + self.num_pos_constraints)
        ...
        self.compute_boundary_vec_batch_single_dof = (jax.vmap(self.compute_boundary_vec_single_dof, in_axes=(
            0)))  # vmap parrallelization takes place over first axis
        self.compute_projection_batched_over_dof = jax.vmap(self.compute_projection_single_dof, in_axes=(0, 0, 0, 0,
                                                                                                         0))  # vmap parrallelization takes place over first axis
    ...
    def compute_rollout_single(self, thetadot, init_pos, init_vel, tray_init):
        ...
        theta, eef_0, eef_vel_lin_0, eef_vel_ang_0, eef_1, eef_vel_lin_1, eef_vel_ang_1, tray, target_0_rot, target_1_rot, collision = out
        return theta.T.flatten(), eef_0, eef_vel_lin_0, eef_vel_ang_0, eef_1, eef_vel_lin_1, eef_vel_ang_1, tray, target_0_rot, target_1_rot, collision

    @partial(jax.jit, static_argnums=(0,))
    def compute_cost_single(self, theta, eef_0, eef_vel_lin_0, eef_vel_ang_0, eef_1, eef_vel_lin_1, eef_vel_ang_1, tray,
                            target_0_rot, target_1_rot, collision, target_0, target_1, target_2, cost_weights):
        # eef_0 = [x, y, z, w, x, y, z]
        ...
        dot_product = jnp.abs(jnp.dot(eef_0[:, 3:] / jnp.linalg.norm(eef_0[:, 3:], axis=1).reshape(1, self.num).T,
                                      target_0[3:] / jnp.linalg.norm(target_0[3:])))
        dot_product = jnp.clip(dot_product, -1.0, 1.0)
        cost_r_0 = 2 * jnp.arccos(dot_product)

        dot_product = jnp.abs(jnp.dot(eef_1[:, 3:] / jnp.linalg.norm(eef_1[:, 3:], axis=1).reshape(1, self.num).T,
        ...
        dot_product = jnp.abs(jnp.dot(tray[:, 3:] / jnp.linalg.norm(tray[:, 3:], axis=1).reshape(1, self.num).T,
        ...
        # Move end effectors to pick orientation
        dot_product = jnp.abs(jnp.dot(eef_0[:, 3:] / jnp.linalg.norm(eef_0[:, 3:], axis=1).reshape(1, self.num).T,
                                      (target_0_rot / jnp.linalg.norm(target_0_rot, axis=1).reshape(1, self.num).T).T))
        dot_product = jnp.clip(dot_product, -1.0, 1.0)
        cost_r_0 = (2 * jnp.arccos(dot_product)) * jnp.identity(self.num)

        dot_product = jnp.abs(jnp.dot(eef_1[:, 3:] / jnp.linalg.norm(eef_1[:, 3:], axis=1).reshape(1, self.num).T,
                                      (target_1_rot / jnp.linalg.norm(target_1_rot, axis=1).reshape(1, self.num).T).T))
        dot_product = jnp.clip(dot_product, -1.0, 1.0)
    ...
    def compute_mean_cov(self, cost_ellite, mean_control_prev, cov_control_prev, xi_ellite):
        ...
        mean_control = (1 - self.alpha_mean) * mean_control_prev + self.alpha_mean * (
                jnp.sum((xi_ellite * w[:, jnp.newaxis]), axis=0) / sum_w)
    ...
    def cem_iter(self, carry, scan_over):
        ...
        theta, eef_0, eef_vel_lin_0, eef_vel_ang_0, eef_1, eef_vel_lin_1, eef_vel_ang_1, tray, target_0_rot, target_1_rot, collision = self.compute_rollout_batch(
            thetadot, init_pos, init_vel, tray_init)
        ...
        carry = (xi_mean, xi_cov, key, state_term, lamda_init, s_init, xi_samples_new, init_pos, init_vel, target_0,
                 target_1, target_2, tray_init, cost_weights, filter)
    ...
    def compute_cem(
        ...
        cost_batch, cost_list_batch, thetadot, theta, avg_res_primal, avg_res_fixed, primal_residuals, fixed_point_residuals, target_0_rot, target_1_rot, eef_0, eef_1 = out
